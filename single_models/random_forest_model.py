...
class _RandomizedDecisionTree:
    """Decision tree variant that samples a feature subset at each split."""
    ...
    def _get_feature_subset(self, n_features):
        if self.max_features == "sqrt":
            ...
            k = max(1, min(n_features, int(n_features * self.max_features)))
        else:
            k = n_features
...
class RandomForestModel:
    """Random forest classifier built from bootstrap-trained trees."""
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None):
        ...
        for i in range(n_estimators):
            tree_seed = int(rng.randint(0, 10**9))
            tree = _RandomizedDecisionTree(
                max_depth=int(self.params.get("max_depth", 10)),
                min_samples_split=int(self.params.get("min_samples_split", 5)),
    ...
    def predict_proba(self, X):
        ...
        all_prob = [tree.predict_proba(X)[:, 1] for tree in self.trees_]
        mean_prob1 = np.mean(np.column_stack(all_prob), axis=1)
        return np.column_stack([1.0 - mean_prob1, mean_prob1])

    def predict(self, X, threshold=0.5):
        prob1 = self.predict_proba(X)[:, 1]
        return (prob1 >= threshold).astype(int)
...
def run_random_forest_experiment(
    ...
    print("Random Forest Experiment (From Scratch)")
    ...
    X_train, y_train, X_test, y_test = load_processed_data(
        train_path=train_path, test_path=test_path, target_column=target_column
    )
...
def get_best_random_forest_predictions(train_path=None, test_path=None, target_column="loan_status"):
    X_train, y_train, X_test, y_test = load_processed_data(
        train_path=train_path, test_path=test_path, target_column=target_column
    )
    model = RandomForestModel(
